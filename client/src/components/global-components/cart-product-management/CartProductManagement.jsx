...
export const CartProductManagement = ({
    ...
    const { isInCart, setIsInCart, quantity, setQuantity } = useContext(Context);
    useEffect(() => {
  ...
  return (
        <Box className="cart-icon-handle">
            {isInCart ? 
                <button className='cursor-pointer p-1' onClick={handleAddToCart}>
                    {t("item.addToCart")}
                </button>
                : 
                <Box className="w-full flex flex-row justify-evenly items-center">
                    <button className='cursor-pointer p-1' onClick={handleIncrease}>
                        <IoMdAdd size={18} />
                    </button>
                    <button className="decrease-trash-cart-item cursor-pointer p-1" onClick={handleTrash}>
